...
Uniswap v4 consolidates all pools into a single `PoolManager` contract and introduces [hooks](/docs/get-started/concepts/hooks) — external contracts that can customize pool behavior at every stage of its lifecycle (initialization, swaps, liquidity changes, fee logic). It uses [flash accounting](/docs/protocols/v4/concepts/flash-accounting) to defer token transfers until the end of a transaction, enabling significant gas savings for multi-step operations.

Uniswap v4 is the most flexible and gas-efficient version of the protocol. If you are building a new integration, start here.
...
Uniswap v2 is the original constant-product AMM (`x * y = k`) with full-range liquidity. It is the simplest version of the protocol and remains deployed on Ethereum mainnet and many other chains.

Uniswap v2 is a good starting point for understanding AMM fundamentals, but new integrations should target v4 unless there is a specific reason to use v2.
