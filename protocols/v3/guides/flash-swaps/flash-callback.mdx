...
Call the first of two swaps, calling `exactInputSingle` on the [**router interface**](/docs/protocols/v3/overview) contract. In this call, we are using the previously declared `amount0In` as the minimum amount out, and assigning the returned balance of the swap to `amountOut0`.
...
If there is any balance due to the token, use simple logic to call [pay](/docs/protocols/v3/overview#pay). Remember that the callback function is being called by the pool itself, which is why we can call `pay` despite the function being marked `internal`.
