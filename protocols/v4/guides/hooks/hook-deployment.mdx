...
As mentioned in [Concept of Hooks](../../concepts/hooks), hook contracts indicate their implemented functions by __encoding its behavior in the address of the contract__. The `PoolManager` uses these permissions to determine which hook functions to call for a given pool. See `PoolManager` deployment addresses [here](/docs/contracts/uniswap-protocol-contracts/v4/deployments).
...
Let's see it in action for a Foundry script. We will make use of the example - Points Hook from [Building Your First Hook](./your-first-hook).
