...
To prevent gas griefing during unsubscription, Uniswap v4 sets a fixed variable [`unsubscribeGasLimit`](/docs/contracts/uniswap-protocol-contracts/v4/reference/periphery/interfaces/INotifier#unsubscribegaslimit) when calling a subscriber’s `notifyUnsubscribe()` function.
...
To opt-in to a subscriber contract, call [`subscribe()`](/docs/contracts/uniswap-protocol-contracts/v4/reference/periphery/interfaces/INotifier#subscribe) on `PositionManager`.
