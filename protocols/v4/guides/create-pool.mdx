...
To initialize a Uniswap v4 Pool _without initial liquidity_, developers should call [`PoolManager.initialize()`](/docs/protocols/v4/concepts/poolmanager#initialize)
...
[PoolKey](/docs/protocols/v4/overview) uniquely identifies a pool
...
Uniswap v4's [PositionManager](/docs/protocols/v4/guides/position-manager) supports atomic creation of a pool and initial liquidity using [_multicall_](/docs/protocols/v4/overview). Developers can create a trading pool, with liquidity, in a single transaction:
...
### 3. Encode the [`initializePool`](/docs/protocols/v4/overview) parameters
Pools are initialized with a starting price
```solidity
...
### 7. Encode the [`modifyLiquidites`](/docs/protocols/v4/guides/position-manager#modifyliquidities) call
```solidity
