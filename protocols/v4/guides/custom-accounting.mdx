...
As described in [Flash Accounting](../concepts/flash-accounting) Uniswap v4 tracks net token transfers with transient storage. Unlike previous versions that tracked absolute token balances, v4 records changes to these balances (*deltas*). This approach is at the core of v4’s enhanced flexibility and efficiency.
...
Custom Curves in Uniswap v4 represent a big change in AMM design flexibility. Unlike [Uniswap v2](../../v2/overview) where the x*y=k formula was hardcoded, v4 allows developers to implement a wide variety of pricing models.
...
<Callout title="Unspecified currency (afterSwap)" type="info">
