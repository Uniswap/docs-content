...
At a high level, in Uniswap V2, _trades must be priced in the periphery_. The good news is that the [library](/docs/protocols/v2/overview)
provides a variety of functions designed to make this quite simple, and all swapping functions in the [router](/docs/protocols/v2/overview) are designed with this in mind.
...
If you'd like to send an exact amount of input tokens in exchange for as many output tokens as possible, you'll want to use [getAmountsOut](/docs/protocols/v2/overview#getamountout). The equivalent SDK function is [getOutputAmount](/docs/sdks/v2/overview#getoutputamount), or [minimumAmountOut](/docs/sdks/v2/overview#minimumamountout-since-204) for slippage calculations.

### Exact Output

If you'd like to receive an exact amount of output tokens for as few input tokens as possible, you'll want to use [getAmountsIn](/docs/protocols/v2/overview#getamountsin). The equivalent SDK function is [getInputAmount](/docs/sdks/v2/overview#getinputamount), or [maximumAmountIn](/docs/sdks/v2/overview#maximumamountin-since-204) for slippage calculations.
