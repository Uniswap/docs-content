...
The most obvious way to get the address for a pair is to call [getPair](/docs/protocols/v2/overview#getpair) on the factory. If the pair exists, this function will return its address, else `address(0)` (`0x0000000000000000000000000000000000000000`).
...
|                        |                                                                                |
| :--------------------- | :----------------------------------------------------------------------------- |
| `address`              | The [factory address](/docs/protocols/v2/overview) |
...
This example makes use of the [Uniswap V2 SDK](/docs/sdks/v2/guides/getting-started). In reality, the SDK computes pair addresses behind the scenes, obviating the need to compute them manually like this.
