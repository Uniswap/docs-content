...
To run this example, check out the guide's [README](https://github.com/Uniswap/examples/blob/main/v3-sdk/price-oracle/README) and follow the setup instructions.

<Callout type="info">
This guide builds on top of the [Pooling Liquidity guides](/docs/sdks/v4/guides/managing-liquidity/position-fetching).
We recommend going through this section of the docs before imnplementing Range Orders.
</Callout>

In this example we will create a single-side liquidity position with the [NonfungiblePositionManager](/docs/protocols/v3/overview) contract.
...
We [create a Pool](/docs/sdks/v3/guides/getting-started) that represents the V3 Pool we are interacting with and get the `token0Price`.
...
If you are not familiar with liquidity Positions, check out the [liquidity position guides](/docs/sdks/v4/guides/managing-liquidity/position-fetching).
...
We can find the Contract address on the official [Uniswap GitHub](https://github.com/Uniswap/v3-periphery/blob/main/deploys).
