...
To fetch all ticks of our Pool, we will use the [Uniswap V3 graph](/docs/resources/subgraphs).
...
The active liqudity at the current Price is also stored in the smart contract - we already fetched it with the `liquidity` function in the [previous guide](/docs/sdks/v3/guides/getting-started).
