...
To prevent gas griefing during unsubscription, Uniswap v4 sets a fixed variable [`unsubscribeGasLimit`](/docs/protocols/v4/overview#unsubscribegaslimit) when calling a subscriber’s `notifyUnsubscribe()` function.
...
To opt-in to a subscriber contract, call [`subscribe()`](/docs/protocols/v4/overview#subscribe) on `PositionManager`.
