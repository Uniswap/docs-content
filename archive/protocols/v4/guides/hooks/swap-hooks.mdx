...
When triggering the `beforeSwap` hook function, there are some parameters we can make use of to customize or extend the behavior of `swap`. These parameters are described in [`beforeSwap`](/docs/protocols/v4/concepts/hooks#beforeswap) from `IHooks`.
...
- `params` The parameters for the swap i.e. [`SwapParams`](/docs/protocols/v4/concepts/poolmanager#swapparams) from `IPoolManager`
...
When triggering the `afterSwap` hook function, there are some parameters we can make use of to customize or extend the behavior of `swap`. These parameters are described in [`afterSwap`](/docs/protocols/v4/concepts/poolmanager#swapparams) from `IHooks`.
