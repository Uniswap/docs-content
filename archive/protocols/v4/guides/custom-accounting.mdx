...
Hook fees often utilize the [`BeforeSwapDelta`](/docs/protocols/v4/overview) mechanism to adjust swap amounts and transfer deltas from the hook to the swap router, enabling precise control over how the fee affects the swap.
...
Custom Curves in Uniswap v4 represent a big change in AMM design flexibility. Unlike [Uniswap v2](/docs/get-started/concepts/amms) where the x*y=k formula was hardcoded, v4 allows developers to implement a wide variety of pricing models.
...
<Callout type="info" title="Unspecified currency (afterSwap)">
