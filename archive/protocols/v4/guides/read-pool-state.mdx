...
Unlike previous versions, v4 uses a different approach for storing and accessing pool data, which requires understanding the use of [`StateLibrary`](/docs/protocols/v4/overview) and [`extsload`](/docs/protocols/v4/overview).
...
While `StateLibrary` deals with persistent storage, [`TransientStateLibrary`](/docs/protocols/v4/overview) is used for handling transient storage. Transient storage, introduced in EIP-1153, is a way to store data that is only needed for the duration of a transaction, making it ideal for temporary data.

It uses the [`exttload`](/docs/protocols/v4/overview) function, which is similar to `extsload`, but for transient storage; it is an external wrapper for the `TLOAD` opcode.
...
For additional reference, see [`StateLibrary`](/docs/protocols/v4/overview) and [`Extsload`](/docs/protocols/v4/overview)
