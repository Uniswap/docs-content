...
The easiest way to safely swap tokens is to use the [router](/docs/protocols/v2/overview), which provides a variety of methods to safely swap to and from different assets. You'll notice that there is a function for each permutation of swapping to/from an exact amount of ETH/tokens.
...
Now that our contract owns 50 DAI, we need to approve to the [router](/docs/protocols/v2/overview) to withdraw this DAI:
...
However, when an off-chain price can't be used, an on-chain oracle should be used instead. Determining the best oracle for a given situation is not a part of this guide, but for more details on the Uniswap V2 approach to oracles, see [Oracles](/docs/protocols/v2/concepts/oracles).
