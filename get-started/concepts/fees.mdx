...
Uniswap v4 removes fixed tiers entirely. Pool creators can set any fee from 0% to 100%, and pools can use [dynamic fees](/docs/protocols/v4/concepts/dynamic-fees) that adjust in real time through [hooks](/docs/protocols/v4/concepts/hooks).
...
With the passage of the [UNIfication governance proposal](https://vote.uniswapfoundation.org/proposals/93) in December 2025, protocol fees are now active on all v2 pools and select v3 pools (At initial enablement, protocol fee activation focused on v2 and selected v3 pools. v4 fee adapter flows are part of the broader protocol-fees architecture and can be enabled through governance-driven expansion). Collected fees flow into a unified onchain system that programmatically burns UNI tokens.
