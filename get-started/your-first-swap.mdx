...
description: Make your first token swap on Uniswap in under 5 minutes using the Uniswap API, SDK, or Solidity.
---
...
<Callout type="info" title="Why Uniswap API?">
The Uniswap API is the fastest way to integrate Uniswap. It handles routing, calldata generation, and can provide gasless swaps via UniswapX through simple HTTP calls from any language.
</Callout>
...
Get the most competitive price for your swap across all available liquidity sources:
...
### Step 4: Execute the Swap with Your Signer
...
The response contains ready-to-broadcast transaction calldata. Your user signs it with their wallet, and the swap executes onchain.

</ImplementationTab>

<ImplementationTab value="sdk">

<Callout type="info" title="SDK vs Uniswap API">
The SDK provides type-safe interfaces and direct wallet integration, but requires more setup. For most use cases, the Uniswap API is simpler and offers more efficient routing plus additional routing options through UniswapX.
</Callout>
...
### Step 4: Get a Route and Execute with Your Signer
...
Use Solidity integration when you need onchain composability, flash swaps, or are building a protocol that must execute swaps atomically within a transaction.
</Callout>
...
- [Explore swap workflow with the Uniswap API](/docs/trading/swapping-api/getting-started)
- [Explore quoting and routing with the Uniswap SDK](/docs/sdks/v4/guides/swapping/quoting)
