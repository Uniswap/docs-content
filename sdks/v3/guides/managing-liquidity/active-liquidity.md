...
To fetch all ticks of our Pool, we will use the [Uniswap V3 graph](../../../../api/subgraph/overview).
...
The active liqudity at the current Price is also stored in the smart contract - we already fetched it with the `liquidity` function in the [previous guide](./pool-data).
...
Now that you are familiar with liquidity data, consider checking out our [next guide](./price-oracle) on using Uniswap as a Price Oracle.
