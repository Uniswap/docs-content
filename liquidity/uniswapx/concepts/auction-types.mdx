---
id: auctiontypes
title: UniswapX Auction Types
description: Compare UniswapX auction mechanisms across Ethereum, Arbitrum, Base, and Unichain using chain-specific timing models.
---
...
<Mermaid
  ...
  A[User inputs swap]
  B[Request For Quote]
  C{Quote better than Uniswap Classic?}
  D[Route to Uniswap Classic]
  E[Send to exclusive filler]
  F[On-chain settlement]
  G[Open Dutch Auction]
  A --> B
  B --> C
  C -->|No| D
  C -->|Yes| E
  ...
  E -->|Fade / decline| G
  G -->|Auction filled| F
  G -->|Auction unfilled: Retry swap| B
  `}
/>

<Callout title="Cosigners" type="note">
Cosigners update auction parameters to reflect real-time prices, compensating for the delay between quoting and signing (which can be up to 30 seconds). They set the auction start block and adjust pricing within the user's signed parameters, while never exceeding the user's slippage tolerance. If you'd like to see how the Cosigner works in practice, please see the technical overview of [UniswapX RFQ](/docs/liquidity/uniswapx/concepts/uniswaprfq).

Currently, the Uniswap Interface and Uniswap API set the cosigner to Uniswap Labs, though this could be updated in the future.
</Callout>
...
TL;DR: **Priority fee bidding system leveraging OP Stack transaction ordering.**
