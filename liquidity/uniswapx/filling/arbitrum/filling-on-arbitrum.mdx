...
Use the [UniswapX SDK](https://github.com/Uniswap/sdks/tree/main/sdks/uniswapx-sdk) to parse the `encodedOrder` field returned from the endpoint. Each of these orders represents a fillable user trade.
...
`DutchV3` orders use a block-based decay mechanism. This design takes advantage of Arbitrum's 250 ms block frequency, allowing for more granular price updates compared to time-based decay systems that are limited by the `block.timestamp` second-level granularity in the EVM.
