...
The `routingPreference` field  previously supported additional values. These additional values are now deprecated (see the [sunset notice](/docs/sunset_notifications/routing_preference_options)). For more information, see the migration section below.
</Callout>

For complete information on the available values and combinations for the `protocols` and `routingPreference` fields, see the field descriptions in the [/quote](/docs/api-references/swapping/quote). A summary is provided below:
