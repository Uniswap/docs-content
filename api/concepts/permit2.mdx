...
The structure of the swap request is defined in the [API documentation](/docs/api-reference/swapping/create_protocol_swap).

There are no additional steps. Everything needed for Permit2 comes directly from the `/quote` response.

If you are unable to support a three-step workflow, we offer a compatibility option for a two-step approve to swap workflow through our [Proxy Approval workflow](/docs/api/concepts/no-permit2-workflow).
