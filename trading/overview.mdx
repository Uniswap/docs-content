---
title: Overview
description: Understand swap integration paths on Uniswap v4 using the Uniswap API, SDK, and smart contracts.
---
...
| **Smart Contracts** | Onchain integrations, Arbitrage | Interacting directly with Router or Pool contracts via Solidity. | High |

## The Uniswap API (Easiest)

The Uniswap API provides a straightforward path to integration. It abstracts away the complexity of finding a competitive route (splitting and hopping between Uniswap v2, v3, v4, and UniswapX) and constructing the transaction calldata.

### How it works
1. **Check Approval**: Use `/check_approval` to see if the user has authorized Permit2 or the router.
2. **Get Quote**: Send a `/quote` request. The API returns a competitive execution path.
3. **Get Transaction**: Send a `/swap` request. The API returns encoded transaction data
4. **Submit**: The user signs the returned transaction, and your app broadcasts it to the chain.
...
If your goal is to **perform swaps from an onchain contract**, for example, as part of an arbitrage bot, a vault strategy, or a DeFi protocol that needs to swap tokens during its operations, you interact with the `Universal Router` or the `PoolManager` directly.
