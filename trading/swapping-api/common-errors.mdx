...
* The `amount` is too low to be quoted by UniswapX. If a quote is requested for only UniswapX (see [Swap Routing](/docs/trading/swapping-api/concepts/swap-routing) for more on protocol selection), and the requested chain is an L2 (e.g. not Mainnet), the amount must exceed a threshold value in order to be quoted (see details in the API reference).
