...
The most commonly encountered HTTP error is an HTTP 404 with a message "No quotes available" from the [`/quote`](/docs/api/guides/swapping/getting-started) endpoint. This error can happen for a variety of reasons related to request syntax, rather than there being no actual route between the specified tokens. In order of most common to less common reasons for receiving this error:
* The `amount` is too low to be quoted by UniswapX. If a quote is requested for only UniswapX (see [Swap Routing](/docs/api/concepts/swap-routing) for more on protocol selection), and the requested chain is an L2 (e.g. not Mainnet), the amount must exceed a threshold value in order to be quoted (see details in the API reference).
