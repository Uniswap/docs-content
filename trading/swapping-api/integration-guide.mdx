...
Full code examples for completing a basic swap workflow are available in [Swapping Code Examples](/docs/api/guides/swapping/swapping-code-examples).
...
- **Allowance Management**: Check and request token approvals (via [Permit2](/docs/api/concepts/permit2))
...
See the [Build Prerequisites](/docs/api/guides/swapping/building-prerequisites) for more details.
...
- Check chains are available for selected protocol (note [limited chain support for UniswapX](/docs/api/supported-chains))
...
- Verify chain IDs are [supported](/docs/api/supported-chains)
